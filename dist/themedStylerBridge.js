...
const usage = {
    tags: new Set(),
    classes: new Set(),
    tagClasses: new Set(), // encoded as `${tag}|${class}`
};
const themes = {};
let currentTheme = null;
export function registerUsage(tag, props, hierarchy) {
...
export function registerTheme(name, defs) {
    ...
    if (typeof globalThis !== 'undefined') {
        ;
        globalThis.__bridgeGetThemes = () => getThemes();
    }
}
export function setCurrentTheme(name) {
    ...
    try {
        ...
        import('./styleManager').then((m) => {
            try {
                m.requestRender && m.requestRender();
            }
            catch { /* noop */ }
        }).catch(() => { });
    }
...
export async function loadThemesFromYamlText(yamlText) {
    try {
        const themeConfig = parseYAML(yamlText);
        ...
        if (themeConfig.default_theme)
            setCurrentTheme(String(themeConfig.default_theme));
    }
    catch (e) {
        console.error('[themedStylerBridge] loadThemesFromYamlText failed:', e);
    }
}
export async function loadThemesFromYamlUrl(url) {
    try {
...
export async function ensureDefaultsLoaded() {
    if (_defaults_loaded)
        return;
...
export function getCssForWeb() {
    // If platform provides a hook, call it
    const g = typeof globalThis !== 'undefined' ? globalThis : {};
    if (typeof g.__themedStylerRenderCss === 'function') {
        try {
            return g.__themedStylerRenderCss(getUsageSnapshot(), getThemes());
        }
        catch (e) { }
    }
    // If running under Node, attempt to call the hook-transpiler CLI to compute CSS
    if (globalThis && globalThis.process && globalThis.process.versions && globalThis.process.versions.node) {
        try {
            // Use temp file for state JSON
            const _req = globalThis.require ? globalThis.require : eval('require');
            const fs = _req('fs');
            const os = _req('os');
            ...
            const repoRoot = path.resolve((globalThis.process && globalThis.process.cwd && globalThis.process.cwd()) || '.');
...
//# sourceMappingURL=themedStylerBridge.js.map